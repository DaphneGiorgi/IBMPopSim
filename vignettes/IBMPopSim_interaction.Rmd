...
package: IBMPopSim
vignette: >
  %\VignetteIndexEntry{IBMPopSim Population with genetically variable traits}
...
Each event is characterized by its intensity and  kernel code, as follows.
...
death_event <- mk_event_interaction( # Event with intensity of type interaction 
  ...
  interaction_code = "double x_I = I.birth_size + g * age(I,t); 
                      ...
                      result = beta * ( 1.- 1./(1. + c * exp(-4. * (x_I-x_J))));" # Implementation of U 
)
```
...
colors <- c(diverging_hcl(n=length(lbls), palette = "Red-Green")) # attributes a color to each subgroup
names(colors) <- lbls
```
...
colors <- c(diverging_hcl(n=length(lbls), palette = "Red-Green")) # attributes a color to each group
names(colors) <- lbls
```
