...
The main idea of the algorithm is to use candidate event times with *constant* intensity $\bar \lambda$ (which corresponds to exponential variables with parameter $\bar \lambda$). $\bar \lambda$ must *dominates* the true event intensity $\lambda^e(I,t)$ for all individuals $I$:
...
1.  **If** $\theta \leq \dfrac{\lambda^e(I,t+\tau^e)}{\bar\lambda}$ **then**   
...
```{r child="../man/fragments/quick_model.Rmd"}
...
```{r evtype, echo=FALSE}
x = c("Birth", "Entry", "Death", "Exit", "Swap", "Custom")
y = c("`birth`", "`entry`", "`death`", "`exit`", "`swap`", "`custom`")
knitr::kable(data.frame(x,y), "pandoc", col.names = c("Event type","`Type`"), caption = "Event types", booktabs = TRUE)
```

```{r evclass, echo=FALSE}
x = c("Poisson", "Inhomogeneous Poisson", "Individual", "Interaction")
y = c("`poisson`", "`inhomogeneous_poisson`", "`individual`", "`interaction`")
knitr::kable(data.frame(x,y), "pandoc", col.names = c("Intensity class", "`Class`"), caption = "Intensity classes", booktabs = TRUE)
```
...
```{r varBirth, echo=FALSE}
Variable = c("`I`","`t`","` pop `","` newI `" , "Model parameters")
Description = c("Current individual","Current time","Current population (vector)","New individual. By default for birth events `newI = I` with `newI.birth = t`","Depends on the model")
knitr::kable(data.frame(Variable,Description), "pandoc", caption = 'C++ variables available for birth and entry events kernel code')
...
```{r varSwap, echo=FALSE}
Variable = c("`I`","`t`","` pop `", "Model parameters")
Description = c("Current individual","Current time","Current population (vector)","Depends on the model")
knitr::kable(data.frame(Variable,Description), "pandoc", caption = 'C++ variables available for swap events kernel code')
...
The `intensity_code` argument is a character string containing few lines C++ code describing the intensity function. The available variables for the intensity code are given in Table \@ref(tab:varInd) :

```{r varInd, echo=FALSE}
Variable = c("`I`","`t`", "Model parameters")
Description = c("Current individual","Current time","Depends on the model")
knitr::kable(data.frame(Variable,Description), "pandoc", caption = 'C++ variables available for individual intensities')
...
The `intensity_code` argument contains few lines of C++ code describing 
the interaction function $U$. The interaction function value has to be stored in a variable called `result`. The available variables for the intensity code are given in Table \@ref(tab:varInt):

```{r varInt, echo=FALSE}
Variable = c("`I`","`J`","`t`", "Model parameters")
Description = c("Current individual","Another individual in the population","Current time","Depends on the model")
knitr::kable(data.frame(Variable,Description), caption = 'C++ variables available for interaction code')
...
params <- list("p_male"= 0.51, 
               "birth_rate" = stepfun(c(15,40),c(0,0.05,0)),"death_rate"=gompertz(0.008,0.02))
...
Table \@ref(tab:logs) describes the elements of the vector `sim_out$logs`containing information on  simulation algorithm:

```{r logs, echo=FALSE}
 Elements= c("`proposed_events`","`effective_events`","`cleanall_counter`","`duration_main_algorithm`")
Description = c("Number of candidate event times proposed during the simulation","Number of events which occured during the simulation","Number of population cleans", "Simulation time")
knitr::kable(data.frame(Elements,Description), caption = 'Logs parameters')
```
...
sim_out_no_birth <- popsim(model, pop, 
                           ...
                           params, age_max = a_max, time = 30) # Simulation without birth events
```
...
Dxt <- death_table(female_pop, ages = 85:90,period = 20:30) # Death table (death age is age at death)
Ext <- exposure_table(female_pop, ages = 85:90,period = 20:30) # Central Exposure-to-risk table
```
...
```{r plot, echo=FALSE, fig.cap="Evolution of probability to be in subgroup 1"}
plot(time_vec[-1],p_1_t, type ="l", xlab= "time",  ylab="")
```
